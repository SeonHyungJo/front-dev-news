...
description: 정규 표현식(Regular Expressions)
# theme: uncover
paginate: true
---
...
_color: #aaa
-->
# <!--fit--> 정규 표현식(Regular Expressions)
...
즉 `텍스트 내에서 특정한 형태나 규칙을 가진 문자열을 찾기 위해 그 형태나 규칙을 나타내는 패턴을 정의하는 것`을 **정규 표현식**이라고 한다.
...
정규식이 쓰이는 곳이 다양했지만 역사적으로 하나의 통일된 표준이 없다.

처음 유닉스 명령줄 도구들에서 사용하던 정규 표현식은  **POSIX 정규식**에 편입되었다. 이후 **POSIX 정규식**은 POSIX BRE(POSIX 기본 정규식)와 POSIX ERE(POSIX 확장 정규식)으로 버전이 나뉘게 된다.

그 외에 BRE를 기본 골격으로 한 **vim 정규식**이 있다. 하지만 시간이 흐르고 개선과 확장을 거듭하면서 이 **vim 정규식** 역시 **POSIX 표준**과는 좀 다른 규격으로 취급이 되었다.

펄의 정규식은 PCRE라는 규격으로 정리되었으며, 이후 많은 프로그래밍 언어들이 이 규격을 차용하거나 계승했다. 여기서 중요한 것은 **일부 차용**이라는 것이다. 이 규격은 방대해서 PCRE를 그대로 가져다 쓰지 않는 이상, 구현할게 너무 많기 때문이다.
...
> [테스트 사이트](https://regexr.com/) / [시각화 사이트](https://regexper.com/)
...
tomato와 potato에 모두 매칭하고 싶다면 `tomato|potato` 라고 쓸 수 있다.

그 외 `[ ... ]`이 있다. 대괄호속에 넣은 문자 중에서 하나에 매칭하는 것이다. 
`[cfh]all` => `call`, `fall`, `hall`에 모두 매치될 수 있다. 

대시(`A-B`)를 통해서 특정 범위를 표현할 수도 있다.
...
선택 패턴 내에서 `^` 이 쓰이면 `not`의 의미가 되며, 문자 뒤에 오는 문자들은 제외하게 된다. 
`[^cfh]all` => `call`, `fall`, `hall`에는 매치하지 않으며 `mall`에는 매치하게 된다.
...
**Group은 전체 패턴 내에서 다시 하나로 묶여지는 패턴 조각을 나타낸다.** 특히 `|` 나 뒤에 나오는 수량 한정자를 Group에 붙이는 형태로 많이 사용되며, 한 번 매치한 Group을 다시 반복하여 사용할 수 있다.
